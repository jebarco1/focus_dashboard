...
import ShowError from '../../../Forms/FormsLayout/FormWizard/NumberingWizard/CommonError/ShowError';
            ...
            <div className="theme-form">
              <div className="wizard-4">
                ...
                <div className="action-bar">
                  ...
                  {level === 4 && (<a href={Href} className={`btn btn-primary`} onClick={() =>toast.success("form submitted successfully")}>{Finish}</a>)}
                  {level !== 5 && (<a href={Href} onClick={handleNextButton} className={`btn btn-primary ${showFinish ? "buttonDisabled" : ""}`}>{Next}</a>)}
                  {level > 1 && (<a href={Href} className={`btn btn-primary `} onClick={handleBackButton}>{Previous}</a>)}
                </div>
              </div>
            </div>
          </div>
